...
export const getEcosystemConfig = async (req: Request, res: Response) => {
    const config = await prisma.ecosystemConfig.findFirst();
    res.status(200).json(config || {});
};

export const updateEcosystemConfig = async (req: Request, res: Response) => {
    const data = req.body;
        ...
        const updated = await prisma.ecosystemConfig.update({
            ...
            data: {
                ...
                subProductIds: Array.isArray(data.subProductIds)
                    ? data.subProductIds.map((id: any) => Number(id))
                    : []
            },
        });
